...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block w-64 bg-slate-900/60 border-r border-slate-800 p-4 shrink-0 min-h-[calc(100vh-65px)]">
        ...
        <div className="mt-8 p-3.5 rounded-xl bg-slate-800/40 border border-slate-800/80 text-xs text-slate-400">
          <p className="font-semibold text-slate-300">HackDevengers 2.0</p>
          <p className="text-[11px] mt-0.5 text-slate-500">Predictive Farm Intelligence</p>
        </div>
      </aside>

      {/* Mobile Bottom Navigation Bar with Safe Area support */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 border-t border-slate-800 px-2 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] backdrop-blur-lg flex items-center justify-around shadow-2xl">
        {[
